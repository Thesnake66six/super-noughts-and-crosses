...
// Colours and rendering settings for a Draw - `Value::Draw`
    pub const COLOUR_DRAW_BG: Color = COLOUR_NONE;
    pub const COLOUR_DRAW_FG: Color = Color { r: 150, g: 150, b: 150, a: 255 };

// Colours and rendering settings for a `Board`
    pub const COLOUR_BOARD_BG: Color = Color { r: 200, g: 200, b: 200, a: 255 };
    pub const BOARD_CELL_MARGIN: f32 = 0.05; 

/// Draws a `Cell::None` or `Value::None`
pub fn draw_none<T:RaylibDraw>(rect: Rectangle, d: &mut T) {
    d.draw_rectangle(rect.x as i32, rect.y as i32, rect.width as i32, rect.height as i32, COLOUR_NONE);
}

/// Draws a `Cell::Player1` or `Value::Player1`
pub fn draw_cross<T:RaylibDraw>(rect: Rectangle, d: &mut T) {
    d.draw_rectangle(rect.x as i32, rect.y as i32, rect.width as i32, rect.height as i32, COLOUR_CROSS_BG);
    ...
    d.draw_line_ex(Vector2{x: ln_x, y: ln_y}, Vector2{x:ln_fx, y:ln_fy}, rect.width * CROSS_THICK, COLOUR_CROSS_FG);
...
pub fn draw_nought<T:RaylibDraw>(rect: Rectangle, d: &mut T) {
    d.draw_rectangle(rect.x as i32, rect.y as i32, rect.width as i32, rect.height as i32, COLOUR_NOUGHT_BG);
...
pub fn draw_draw<T:RaylibDraw>(rect: Rectangle, d: &mut T) {
    d.draw_rectangle(rect.x as i32, rect.y as i32, rect.width as i32, rect.height as i32, COLOUR_DRAW_BG);
    
    let s = rect.height / 7.0;

    d.draw_rectangle((rect.x + s) as i32, (rect.y + s) as i32, (rect.width - s * 2.0) as i32, (s) as i32, COLOUR_DRAW_FG);
    d.draw_rectangle((rect.x + s) as i32, (rect.y + 3.0 * s) as i32, (rect.width - s * 2.0) as i32, (s) as i32, COLOUR_DRAW_FG);
    d.draw_rectangle((rect.x + s) as i32, (rect.y + 5.0 * s) as i32, (rect.width - s * 2.0) as i32, (s) as i32, COLOUR_DRAW_FG);
}